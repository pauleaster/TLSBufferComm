...
# prompt> . ./keys/generate_receiver_keys_bash.sh
# note the initial "." and the space between the two paths!!!
# or 
# prompt> . ./keys/generate_receiver_keys_bash.sh mydomain.com
...
certificate_file="$ssl_directory/eb_receiver_certificate.pem"
echo "$certificate_data" > "$certificate_file"

# Write the private key data to a file
private_key_file="$ssl_directory/eb_receiver_private_key.pem"
echo "$private_key_data" > "$private_key_file"

# Set the environment variables
export EB_RECEIVER_CERTIFICATE_DATA="$certificate_data"
export EB_RECEIVER_PRIVATE_KEY_DATA="$private_key_data"
