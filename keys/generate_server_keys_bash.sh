...
# prompt> . ./keys/generate_server_keys_bash.sh
# note the initial "." and the space between the two paths!!!
# or 
# prompt> . ./keys/generate_server_keys_bash.sh mydomain.com
...
certificate_file="$ssl_directory/eb_server_certificate.pem"
echo "$certificate_data" > "$certificate_file"

# Write the private key data to a file
private_key_file="$ssl_directory/eb_server_private_key.pem"
echo "$private_key_data" > "$private_key_file"

# Set the environment variables
export EB_SERVER_CERTIFICATE_DATA="$certificate_data"
export EB_SERVER_PRIVATE_KEY_DATA="$private_key_data"

# Append the environment variable definitions to the fish config file
echo "set -xg EB_SERVER_CERTIFICATE_DATA '$certificate_data'" >> ~/.config/fish/config.fish
echo "set -xg EB_SERVER_PRIVATE_KEY_DATA '$private_key_data'" >> ~/.config/fish/config.fish
